...
for i in range(0, grid.shape[0]):
    ...
    visited_squares = []
    q = deque([(i, 0, Direction.RIGHT)])
    while q:
        value = q.popleft()
        process_laser(q, grid, energized_grid, visited_squares, value[0], value[1], value[2])
    energized_sums.append(np.sum(energized_grid))

    energized_grid = np.zeros_like(grid, dtype=int)
    visited_squares = []
    q = deque([(i, grid.shape[1] - 1, Direction.LEFT)])
    while q:
        value = q.popleft()
        process_laser(q, grid, energized_grid, visited_squares, value[0], value[1], value[2])
    energized_sums.append(np.sum(energized_grid))
...
for i in range(0, grid.shape[1]):
    ...
    energized_grid = np.zeros_like(grid, dtype=int)
    visited_squares = []
    q = deque([(grid.shape[0] - 1, i, Direction.UP)])
    while q:
        value = q.popleft()
        process_laser(q, grid, energized_grid, visited_squares, value[0], value[1], value[2])
    energized_sums.append(np.sum(energized_grid))

print(max(energized_sums))
